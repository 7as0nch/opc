...
// 准星光标（移植自 ReactBits TargetCursor）。
// 在 a / button / .cursor-target 等目标上锁定为四角准星，其余时间缓慢自旋。
// 移动端直接返回 null 且不隐藏原生光标。固定类样式在 globals.css（组件用 querySelector 取角，故不可 hash 化）。
...
// position:fixed 元素相对视口定位，除非祖先建立了包含块（transform/perspective/filter/will-change/contain）。
// 出现包含块时，需测量并补偿其偏移，否则平移坐标会错位。
